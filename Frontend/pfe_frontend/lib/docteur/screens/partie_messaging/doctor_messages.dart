...
class DoctorMessage extends StatefulWidget {
  const DoctorMessage({Key? key }) : super(key: key);
  @override
  ...
  _setListPatient() async 
  {
      if(!reservations.isEmpty){
        for( var i = 0 ; i < reservations.length; i++ ) { 
          ...
          User user1 = await ApiMethods().getUserById(reservations[i].patient_id);
          if(!patientList.isEmpty){
            for( var j = 0 ; j < patientList.length; j++ ) {
              if(patientList[j].id != reservations[i].docteur_id )
                  ...
                  ? ListView.builder(
                      ...
                      itemBuilder: (context, index) => Card(
                        ...
                        child: ListTile(
                          ...
                          onTap: () {
                            launchWhatsApp(patientList[index].mobilenumber);
                          },
                        ),
                      ),
                    )
